...
def add_jdejob(fullpath,newjob, extendedinfo, ube,version, rtu_id, agent_id):
    job_id = newjob['id']
    envfile = newjob['environmentfile']
    newjob['id']='0'
    jobid_adapt , jobdata_adapt = tesconn.getJobbyAlias(f'{job_id}_adapt')
    if jobid_adapt != None:
...
def add_jde_jobs():
    logging.basicConfig(filename='JDE_SC.log', force=True, encoding='utf-8', level=logging.DEBUG if cfg.DEBUG else logging.INFO )
    logging.info(f'Start Update Jobs based onselection criteria')
...
'''
            ...
            if j.environmentfile in envfile_mapping:
                ...
                continue
            jobid, jobdata = tesconn.getJobbyAlias(j.alias)
            jobid, jobdata = tesconn.getJob(f"{j.name}",j.parentname)
            jobid_adapt , jobdata_adapt = tesconn.getJobbyAlias(f'{j.id}_adapt')
        ...
        '''
                    add_jdejob(j.fullpath,jobdata, jdexml,ube=ube,version=version, rtu_id=rtu_id, agent_id=agent_id)
            else:
...
job_data, jobid = tesconn.getJob('1 Delete Records from F03B13Z1 (R03B0041A PS00001)', '\\19:30 Corporate Cleanup')
